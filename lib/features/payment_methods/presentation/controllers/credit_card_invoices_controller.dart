...
final creditCardInvoicesControllerProvider =
    StateNotifierProvider<CreditCardInvoicesController, CreditCardInvoicesState>((ref) {
  final httpClient = ref.watch(appHttpClientProvider);
  return CreditCardInvoicesController(httpClient);
});

class CreditCardInvoicesController extends StateNotifier<CreditCardInvoicesState> {
  final AppHttpClient httpClient;

  CreditCardInvoicesController(this.httpClient) : super(const CreditCardInvoicesState.initial());
  ...
  Future<void> getInvoices(String paymentMethodId) async {
    ...
    try {
      final response = await httpClient.get('/credit-card-invoices/payment-method/$paymentMethodId?isClosed=false');
      final List<dynamic> responseList = response.data as List<dynamic>;
      final invoices = responseList.map((json) => CreditCardInvoice.fromJson(json)).toList();
      // Ordena as faturas por data de vencimento (mais recente primeiro)
